...
/**
 * User: lgueye Date: 16/02/12 Time: 19:12
 */
@Component
@Provider
public class GenericExceptionMapper implements ExceptionMapper<Throwable> {
  ...
  public Response toResponse(final Throwable th) {
    ...
    final
    ResponseError
        error = this.exceptionConverter.toResponseError(th, this.request);
    ...
    if (StringUtils.isNotEmpty(preferredResponseMediaType)
      ...
      return Response.status(error.getHttpStatus())
          .header("Content-Type", ExceptionConverter.DEFAULT_MEDIA_TYPE)
          .entity(error).build();
    }
